...
async def validate_report(
    ...
    execute_write(conn, """
        ...
        SET status          = %s,
            ...
            validated_at    = NOW()
        WHERE report_id = %s
    """, (data.status, current_user['id'], report_id))
...
async def download_report_pdf(
    ...
    try:
        ...
        cert_text = (
          f"This report was generated by {clean(report.get('inspector_name'))} "
          f"and encrypted with AES-256 at source. "
        )
        
        if report.get('status') == 'validated' and report.get('validated_by_name'):
             cert_text += (
                 f"It was officially validated by {clean(report.get('validated_by_name'))} "
                 f"on {clean(str(report.get('validated_at')))}. "
             )
        
        cert_text += (
          f"Content integrity is guaranteed by the Taroudant Heritage Shield "
          f"monitoring system. Any unauthorized modification will invalidate this document."
        )

        pdf.multi_cell(0, 6, cert_text)
        pdf.ln(8)
