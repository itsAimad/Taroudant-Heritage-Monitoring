          ...
          >
            ...
            {status === 'success' ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  <label className="block text-xs font-medium tracking-[0.18em] text-sand/50 uppercase mb-1.5">
                    Phone Number <span className="normal-case text-sand/30">(optional)</span>
                  </label>
                  <input
                    type="tel"
                    value={form.phone}
                    onChange={(e) => handleChange('phone', e.target.value)}
                    className="w-full rounded-md border border-sand/10 bg-sand/5 px-4 py-2.5 text-sm text-sand placeholder:text-sand/25 focus:outline-none focus:border-copper-light/40 focus:bg-sand/8 transition-colors duration-200"
